-- Update member_balances to include settlements so the view reflects
-- fully-settled balances. Using CTEs to avoid cartesian products when
-- joining both settlement directions simultaneously.
...
settlement_out AS (
    -- Settlements paid by this member increase their balance (reduce their debt)
    SELECT from_member AS member_id, group_id, currency, SUM(amount) AS total
    FROM settlements
    GROUP BY from_member, group_id, currency
),
settlement_in AS (
    -- Settlements received by this member decrease their balance (they've been repaid)
...
SELECT
    ...
    eb.expense_net
        + COALESCE(so.total, 0)
        - COALESCE(si.total, 0) AS net_balance
