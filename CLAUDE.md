...
`design-system/` is the Chara design system as consumed by Claude Design (project
"Chara Design System"). It is a **projection of `app/lib/theme.ts`**, never a second
source of truth: `design-system/tokens.css` is generated by
`app/lib/__tests__/design-tokens.test.ts`, which fails when the two drift. After
changing a token, regenerate and re-upload:

```
cd app && UPDATE_DESIGN_TOKENS=1 pnpm test design-tokens
```

The hand-authored tail of `tokens.css` (component classes, `@font-face`) sits below a
sentinel comment and is left alone by the generator. `preview/*.html` and
`ui_kits/app/*.html` each declare their own Design System card on line 2 via an
`@dsCard` comment; `_ds_manifest.json` is compiled server-side, so don't author it.
