    ...
    <div className="min-h-screen bg-indigo">
      ...
      <LegalPrint index="lg.01" title="Privacy" updated="26 May 2026">
        ...
        <ul className="list-disc pl-6 space-y-2">
          ...
          <li><strong>Voice input</strong> — if you choose to add an expense by speaking, audio is recorded only while you hold the control, and only in the group you are adding to. The recording is sent to Google's Gemini API to be transcribed and turned into a draft expense. We hold it in memory for that single request: it is never written to disk, never stored in our file storage, and never written to our logs. The transcript is returned to your device so you can correct it, and is not stored on our servers. Only the resulting expense — the one you review and save — is stored.</li>
